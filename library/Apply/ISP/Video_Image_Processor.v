...
module Video_Image_Processor #(
	...
	//全局时钟
	input				clock,  				//cmos视频像素时钟
	input				reset,				//全局复位
	//待处理的图像数据
	input				per_img_vsync,	//准备图像数据vsync有效信号
	input				per_img_href,		//准备图像数据href有效信号
	input		[7:0]	per_img_RAW,		//准备图像数据 YCbCr 4:2:2 {CbY} {CrY}
	//Image data has been processd
	output				post_img_vsync,	//处理后图像数据vsync有效信号
	output				post_img_href,	//处理后图像数据href有效信号
	output		[7:0]	post_img_Gray,		//处理后图像灰度输出

	//user interface
	input		[7:0]	Sobel_Threshold		//图像边缘检测的Sobel阈值	
);

//-------------------------------------
//将RAW格式转换为RGB888格式。
wire				post1_frame_vsync;	//处理后图像数据vsync有效信号
wire				post1_frame_href;	//处理后图像数据href有效信号
...
wire				post2_frame_vsync;	//处理后图像数据vsync有效信号
wire				post2_frame_href;	//处理后图像数据href有效信号
wire		[7:0]	post2_img_Gray;		//处理后图像灰度输出
...
generate if(RAW2RG888_YUV444 == "ON") begin : RAW2RG888_YUV444_ON
	RAW8_RGB888	# (
		...
		//全局时钟
		.clock					(clock),					//cmos视频像素时钟
		.reset					(reset),				//系统复位

		//Image data prepred to be processd
		.per_img_vsync		    (per_img_vsync),		//准备图像数据vsync有效信号
		.per_img_href			(per_img_href),		//准备图像数据href有效信号
		.per_img_RAW			(per_img_RAW),			//Prepared Image data 8 Bit RAW Data
		
		//Image data has been processd
		.post_img_vsync		    (post1_frame_vsync),	//处理后图像数据vsync有效信号
		.post_img_href		    (post1_frame_href),		//处理后图像数据href有效信号
	...
	RGB888_YCbCr444	RGB888_YCbCr444_u(
		...
		//Image data prepred to be processd
		.per_img_vsync	(post1_frame_vsync),	//准备图像数据vsync有效信号
		.per_img_href		(post1_frame_href),		//准备图像数据href有效信号
		...
		//Image data has been processd
		.post_img_vsync	(post2_frame_vsync),	//Processed Image frame data valid signal
		.post_img_href	(post2_frame_href),		//Processed Image hsync data valid signal
	...
	assign	post2_frame_vsync = per_img_vsync;
	assign	post2_frame_href  = per_img_href;
...
wire				post3_frame_vsync;	//处理后图像数据vsync有效信号
wire				post3_frame_href;	//处理后图像数据href有效信号
wire		[7:0]	post3_img_Gray;		//处理后图像灰度输出
generate if(GRAY_MEDIUM == "ON") begin : GRAY_MEDIUM_ON
	Gray_Median_Filter # (
		...
		//Image data prepred to be processd
		.per_img_vsync		    (post2_frame_vsync),		//准备图像数据vsync有效信号
		.per_img_href			(post2_frame_href),		    //准备图像数据href有效信号
		.per_img_Gray			(post2_img_Gray),			//Prepared Image brightness input

		//Image data has been processd
		.post_img_vsync		    (post3_frame_vsync),	//处理后图像数据vsync有效信号
		.post_img_href		    (post3_frame_href),		//处理后图像数据href有效信号
		.post_img_Gray			(post3_img_Gray)		//Processed Image brightness output
	);
end
else if(GRAY_MEDIUM == "OFF") begin : GRAY_MEDIUM_OFF
	assign	post3_frame_vsync = post1_frame_vsync;
	assign	post3_frame_href  = post1_frame_href;
...
wire			post4_frame_vsync;	//处理后图像数据vsync有效信号
wire			post4_frame_href;	//处理后图像数据href有效信号
...
generate if(GRAY_SOBEL == "ON") begin : GRAY_SOBEL_ON
	Sobel_Edge_Detector # (
		...
		//Image data prepred to be processd
		.per_img_vsync		(post3_frame_vsync),	    //准备图像数据vsync有效信号
		.per_img_href			(post3_frame_href),		//准备图像数据href有效信号
		.per_img_Gray			(post3_img_Gray),		//Prepared Image brightness input

		//Image data has been processd
		.post_img_vsync		(post4_frame_vsync),	//处理后图像数据vsync有效信号
		.post_img_href		(post4_frame_href),		//处理后图像数据href有效信号
		.post_img_Bit		(post4_img_Bit),		//Processed Image Bit flag outout(1: Value, 0:inValid)
		
		//User interface
		.Sobel_Threshold		(Sobel_Threshold)		//图像边缘检测的Sobel阈值
	);
...
wire			post5_frame_vsync;	//处理后图像数据vsync有效信号
wire			post5_frame_href;	//处理后图像数据href有效信号
...
generate if(GRAY_EROSION == "ON") begin : GRAY_EROSION_ON
	Bit_Erosion_Detector # (
		...
		//Image data prepred to be processd
		.per_img_vsync		(post4_frame_vsync),	//准备图像数据vsync有效信号
		.per_img_href			(post4_frame_href),		//准备图像数据href有效信号
		.per_img_Bit			(post4_img_Gray[0]),	//Processed Image Bit flag outout(1: Value, 0:inValid)

		//Image data has been processd
		.post_img_vsync		(post5_frame_vsync),	//处理后图像数据vsync有效信号
		.post_img_href		(post5_frame_href),		//处理后图像数据href有效信号
...
wire			post6_frame_vsync;	//处理后图像数据vsync有效信号
wire			post6_frame_href;	//处理后图像数据href有效信号
...
generate if(GRAY_DILATION == "ON") begin : GRAY_DILATION_ON
	Bit_Dilation_Detector # (
		...
		//Image data prepred to be processd
		.per_img_vsync		(post5_frame_vsync),	//准备图像数据vsync有效信号
		.per_img_href			(post5_frame_href),		//准备图像数据href有效信号
		.per_img_Bit			(post5_img_Gray[0]),	//Processed Image Bit flag outout(1: Value, 0:inValid)

		//Image data has been processd
		.post_img_vsync		(post6_frame_vsync),	//处理后图像数据vsync有效信号
		.post_img_href		(post6_frame_href),		//处理后图像数据href有效信号
...
assign	post_img_vsync = post6_frame_vsync;	//处理后图像数据vsync有效信号
assign	post_img_href  = post6_frame_href;	//处理后图像数据href有效信号
assign	post_img_Gray    = post6_img_Gray;		//处理后图像灰度输出	
