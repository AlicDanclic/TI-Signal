...
reg  [1:0]  per_frame_href_d,per_frame_vsync_d;
always @ (posedge clock or posedge reset) begin
	if(reset) begin
		per_frame_href_d  <= 2'b0;
		per_frame_vsync_d <= 2'b0;
	end
	else begin
		per_frame_href_d  <= {per_frame_href_d[0],per_img_href};
		per_frame_vsync_d <= {per_frame_vsync_d[0],per_img_vsync};
	end
end
assign href_pos  =(~per_frame_href_d[1]  & per_frame_href_d[0])  ? 1'b1 : 1'b0; //一行信号的开始
assign href_neg  =(~per_frame_href_d[0]  & per_frame_href_d[1])  ? 1'b1 : 1'b0; //一行信号的结束
assign vsync_neg =(~per_frame_vsync_d[0] & per_frame_vsync_d[1]) ? 1'b1 : 1'b0; //一帧信号的结束
