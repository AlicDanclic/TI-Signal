...
This standalone file is the canonical CV-R20 Raspberry Pi deployment.  It
...
TASK5_CV_BUILD = "2026-08-02-lock-beep-quick-target-freeze-r21"
TASK5_CV_BUILD_TAG = "CV-R21"
...
def circle_fit_is_locked(
    ...
    circle = target.get("circle_lock", {})
    return (
        fit.quality >= int(circle.get("lock_quality", target.get("lock_quality", 65))) and
        abs(fit.span_y_div - fit.span_x_div) <= float(
            circle.get("maximum_span_delta_div", 0.75)) and
        fit.radial_cv <= float(circle.get("maximum_radial_cv", 0.20)) and
        fit.ellipse_axis_ratio <= float(circle.get("maximum_axis_ratio", 1.25)) and
        fit.inner_fill_ratio <= float(circle.get("maximum_inner_fill_ratio", 0.32)) and
        fit.fill_ratio <= float(circle.get("maximum_fill_ratio", 0.35)) and
        fit.angular_coverage >= float(circle.get("minimum_angular_coverage", 0.62)) and
        min(fit.span_x_div, fit.span_y_div) >= float(
            circle.get("minimum_span_div", 6.0)) and
        max(fit.span_x_div, fit.span_y_div) <= float(
            circle.get("maximum_span_div", 9.4)) and
        fit.center_error_div <= float(
            circle.get("maximum_center_error_div", 1.0))
    )
...
def circle_fit_is_fast_circle_lock(
    ...
    return (
        ...
        fit.ellipse_axis_ratio <= float(
            circle.get("fast_lock_maximum_axis_ratio", 1.35)) and
        abs(fit.span_y_div - fit.span_x_div) <= float(
            circle.get("fast_lock_maximum_span_delta_div", 0.80))
    )
...
def circle_fit_is_quick_locked(
    ...
    return (
        ...
        fit.quality >= int(circle.get("quick_lock_quality", 58)) and
        fit.ellipse_axis_ratio <= float(circle.get(
            "quick_lock_maximum_axis_ratio", 1.70)) and
        fit.center_error_div <= float(circle.get(
            "quick_lock_maximum_center_error_div", 1.50)) and
...
def circle_fit_lock_failures(
    ...
    circle = target.get("circle_lock", {})
    failures: list[str] = []
    if fit.quality < int(circle.get(
            "lock_quality", target.get("lock_quality", 65))):
        failures.append("QUALITY")
    if abs(fit.span_y_div - fit.span_x_div) > float(circle.get(
            "maximum_span_delta_div", 0.75)):
        failures.append("SPAN_DELTA")
    if fit.radial_cv > float(circle.get("maximum_radial_cv", 0.20)):
        failures.append("RADIAL_CV")
    if fit.ellipse_axis_ratio > float(circle.get("maximum_axis_ratio", 1.25)):
    ...
    if fit.fill_ratio > float(circle.get("maximum_fill_ratio", 0.35)):
    ...
    if max(fit.span_x_div, fit.span_y_div) > float(circle.get(
            "maximum_span_div", 9.4)):
        failures.append("SPAN_LARGE")
    if fit.center_error_div > float(circle.get(
...
def target_fit_is_quick_locked(
    ...
    return (
        fit.quality >= quality and
        fit.desired_score <= float(tracking.get(
            "quick_lock_maximum_desired_score", 0.070)) and
        abs(target_phase_delta(fit.estimated_phase, target)) <= int(
            tracking.get("quick_lock_phase_deadband", 12)) and
...
class AutoLissajousController:
    ...
    def _start_circle_sweep(self, now: float) -> None:
        ...
        LOGGER.info(
            "target sweep R20 target=%d measured=%.1fkHz grid=%.1fkHz "
    ...
    def _finish_circle_sweep_candidate(self, now: float) -> None:
        ...
        if fast_seed_kind:
            ...
            if fast_seed_kind == "ellipse":
                LOGGER.info(
                    "R20 fast ellipse seed latched %.3fkHz axis=%.3f "
                ...
                LOGGER.info(
                    "R20 fast control seed latched %.3fkHz trace_q=%d "
    ...
    def _restore_latched_fast_circle_seed(
        ...
        LOGGER.warning(
            "R20 retained fast ellipse seed %.3fkHz after %s; "
    ...
    def _capture_circle_confirm_frame(self, now: float) -> None:
        ...
        shape_locked = (
            (circle_fit_is_fast_circle_lock(
                circle_fit, foreground_occupancy, self.config)
             if fast_circle else
             circle_fit_is_locked(circle_fit, self.config))
 ...
 'target': {'initial_amplitude': {'1': 103, '2': 103, '3': 103},
            ...
            'lock_quality': 65,
             ...
             'circle_lock': {'screen_size': [640, 512],
                             'settle_s': 0.38,
                             'minimum_pixels': 80,
                             # main() enables this through the R20 runtime
...
def enforce_r17_runtime_contract(config: dict[str, Any]) -> list[str]:
    """Keep stale site YAML compatible with the R20 latched circle path."""
    ...
    protected_sweep = {
        "high_frequency_positive_first": True,
        "high_frequency_screen_step_hz": 100.0,
        "high_frequency_tier_radii_hz": [500.0, 1000.0, 1500.0, 2500.0],
        "high_frequency_required_positive_search_hz": 2500.0,
        "high_frequency_screen_settle_s": 0.30,
        "high_frequency_screen_frames_per_candidate": 4,
        "high_frequency_screen_minimum_frames": 3,
...
def main() -> int:
    ...
    if guarded_keys:
        logging.warning(
            "CV-R20 replaced stale safety-critical overrides: %s",
            ", ".join(guarded_keys),
        )
    sweep_config = config["target"]["circle_sweep"]
    logging.info(
        "CV-R20 effective scan step=%.0fHz positive_first=%s radii=%s "
